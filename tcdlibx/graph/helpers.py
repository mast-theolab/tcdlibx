...
def write_molecule_pov(mol_myvtkactor: 'MyvtkActor', filename: str) -> None:
    """Write molecular information as a separate POV-Ray file using estampes.

    Creates a companion file named ``<stem>_mol.pov`` alongside the VTK scene
    file *filename*, containing the molecular representation (atoms and bonds)
    generated by ``estampes.visual.povrender``.
    ...
    Args:
        ...
        filename: Path to the VTK scene .pov file; the molecule POV file is
                  written as ``<stem>_mol.pov`` in the same directory.
    """
    base, ext = os.path.splitext(filename)
    mol_filename = base + '_mol' + (ext if ext else '.pov')
    ...
    with open(mol_filename, 'w', encoding='utf-8') as fobj:
        write_pov_head(fobj, cam_depth)
